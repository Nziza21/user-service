...
func main() {
	...
	productHandler := myhttp.NewProductHandler(productService)

	r := router.SetupRouter(userHandler, authHandler, productHandler, jwtSecret)
	...
	if err := r.Run(":" + cfg.Port); err != nil {
		log.Fatal("Failed to start server1:", err)
	}
}
